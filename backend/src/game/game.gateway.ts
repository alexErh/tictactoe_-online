...
export class GameGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleJoinQueue(client: Socket, nickname: string) {
    const gameStatus = await this.gameService.joinPlayersQueue(
      client.id,
      nickname,
    );
    ...
    if (gameStatus) {
      if (gameStatus instanceof GameStatusDto) {
        this.server
          .to(gameStatus.player1.clientId)
          .emit('newState', gameStatus);
  ...
  async handleMove(client: Socket, data: GameStatusDto) {
    ...
    if (winner === null) {
      ...
      if (data.player1.symbol === winner) {
        ...
        data.winner = winner; // Bei Unentschieden
      }
      ...
      // Gewinner im Spielservice festlegen
      await this.gameService.setWinner(update);

      // Spieler mit dem Gewinner benachrichtigen
